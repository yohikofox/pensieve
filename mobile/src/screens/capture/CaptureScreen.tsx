...
const CaptureScreenContent = () => {
  ...
  const [showStorageWarningDialog, setShowStorageWarningDialog] =
    useState(false);
  const [showSavedDialog, setShowSavedDialog] = useState(false);
  const [lastSavedCaptureId, setLastSavedCaptureId] = useState<string | null>(
    null,
  );
  ...
  const transcriptionEngineServiceRef =
    useRef<TranscriptionEngineService | null>(null);
  ...
  useEffect(() => {
    ...
    repositoryRef.current = container.resolve<ICaptureRepository>(
      TOKENS.ICaptureRepository,
    );
    permissionServiceRef.current = container.resolve<IPermissionService>(
      TOKENS.IPermissionService,
    );
    ...
    transcriptionEngineServiceRef.current = container.resolve(
  ...
  useEffect(() => {
    const performCrashRecovery = async () => {
      ...
      if (recovered.length > 0) {
        const recoveredCount = recovered.filter(
          (r) => r.state === "recovered",
        ).length;
        const failedCount = recovered.filter(
          (r) => r.state === "failed",
        ).length;
    ...
    if (engineService && modelService) {
      try {
        ...
        if (isNativeEngine) {
          ...
          console.log(
            "[CaptureScreen] Native transcription engine selected, skipping Whisper model check",
          );
        } else {
          ...
          if (!bestModel) {
            ...
            console.log(
              "[CaptureScreen] Whisper engine selected but no model available, showing prompt",
            );
        ...
        console.error(
          "[CaptureScreen] Failed to check transcription engine/model:",
          error,
        );
  ...
  const stopRecording = async () => {
    ...
    if (stopResult.type !== "success" || !stopResult.data) {
      console.error(
        "[CaptureScreen] Failed to stop recording:",
        stopResult.type,
        stopResult.error,
      );
    ...
    if (fileStorageServiceRef.current && repositoryRef.current) {
      ...
      const updateResult = await repositoryRef.current.update(
        stopResult.data.captureId,
        {
          state: "captured",
    ...
    toast.success(
      t("capture.alerts.savedVoice", {
        duration: Math.floor(stopResult.data.duration / 1000),
      }),
    );
  ...
  const cancelRecording = async () => {
    ...
    try {
      ...
      console.error(
        "[CaptureScreen] Failed to stop audio recorder during cancel:",
        error,
      );
    }
    ...
    if (result.type !== "success") {
      console.error(
        "[CaptureScreen] Failed to cancel recording:",
        result.type,
        result.error,
      );
  ...
  const handleContinueWithoutTranscription = async () => {
    ...
    console.log(
      "[CaptureScreen] User chose to continue without transcription model",
    );
  ...
  const proceedWithRecording = async () => {
    ...
    try {
      ...
      if (result.type !== "success") {
        console.error(
          "[CaptureScreen] Failed to start recording:",
          result.type,
          result.error,
        );
