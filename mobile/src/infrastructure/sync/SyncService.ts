...
export class SyncService {
  ...
  private lazyAudioDownloader = new LazyAudioDownloader();
    ...
    for (const [entity, entityChanges] of Object.entries(changes)) {
      ...
      if (entityChanges.updated) {
        for (const record of entityChanges.updated) {
          ...
          // Après upsert d'une capture audio : synchroniser raw_content vers un path local (fire-and-forget)
          if (entity === 'captures' && localRecord.type === 'audio') {
            this.lazyAudioDownloader.syncAudioForCapture(localRecord.id as string).catch((err) =>
              console.warn(`[Sync] Audio sync failed for ${localRecord.id}:`, err)
            );
          }
        }
      }
  ...
  private mapCaptureFromBackend(record: any): Record<string, string | number | null> {
    ...
    // raw_content : source de vérité = filesystem local (device-only).
    // Pour les captures audio, le backend renvoie le path Supabase (ex: audio/<userId>/<file>.m4a)
    // qui n'est PAS exploitable directement sur le device. Ne jamais l'écrire tel quel.
    // syncAudioForCapture (appelé après upsert) résoudra la valeur correcte en local.
    // Pour les captures texte, rawContent est le contenu textuel → toujours inclus.
    const isAudioCapture = (record.typeName ?? 'audio') === 'audio';

    if (record.rawContent != null && record.rawContent !== '') {
      if (isAudioCapture) {
        // Alerter si le backend a envoyé une valeur non-locale (path Supabase)
        const isLocalPath = record.rawContent.startsWith('file://') || record.rawContent.startsWith('/');
        if (!isLocalPath) {
          console.warn(
            `[Sync] ⚠️ rawContent non-local reçu du backend pour capture audio ${record.clientId}: "${record.rawContent}" — ignoré, sera résolu par syncAudioForCapture`
          );
          // Ne pas inclure raw_content : syncAudioForCapture écrira le path local correct
        } else {
          // Path local valide reçu du backend (cas rare) → utiliser
          mapped.raw_content = record.rawContent;
        }
      } else {
        // Capture texte : rawContent = contenu textuel → utiliser directement
        mapped.raw_content = record.rawContent;
      }
    }

    // audioUrl is the presigned MinIO URL generated by the backend (valid 24h).
    // Stored locally so LazyAudioDownloader can fetch the file after reinstall.
    if (record.audioUrl != null && record.audioUrl !== '') {
      mapped.audio_url = record.audioUrl;
    }
