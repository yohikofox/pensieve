...
defineFeature(feature, (test) => {
  ...
  const givenAuthenticatedUser = (given: any) => {
    given(/un utilisateur authentifié avec l'ID "(.*)" et l'email "(.*)"/, async (userId: string, email: string) => {
      // Create test user
  ...
  test('Récupération des permissions utilisateur avec succès', ({ given, when, then, and }) => {
    givenAuthenticatedUser(given);

    given(/l'utilisateur "(.*)" a la permission "debug_mode_access" définie à "(.*)"/, async (userId: string, value: string) => {
      const debugModeAccess = value === 'true';
      await dataSource.getRepository(User).update(
        { id: userId },
        { debug_mode_access: debugModeAccess }
      );
    });

    when(/l'utilisateur requête "GET \/api\/users\/(.*?)\/features"/, async (userId: string) => {
  ...
  test('Récupération des permissions avec debug_mode_access activé', ({ given, when, then, and }) => {
    givenAuthenticatedUser(given);
  ...
  test('Accès refusé sans authentification', ({ when, then }) => {
    when(/un utilisateur non authentifié requête "GET \/api\/users\/(.*?)\/features"/, async (userId: string) => {
      response = await request(app.getHttpServer())
        .get(`/api/users/${userId}/features`);
  ...
  test('Un utilisateur ne peut pas accéder aux permissions d\'un autre utilisateur', ({ given, when, then, and }) => {
    givenAuthenticatedUser(given);

    given(/un autre utilisateur existe avec l'ID "(.*)"/, async (otherUserId: string) => {
    ...
    when(/l'utilisateur "(.*)" requête "GET \/api\/users\/(.*?)\/features"/, async (requestingUserId: string, targetUserId: string) => {
      response = await request(app.getHttpServer())
    ...
    and(/la réponse contient un message d'erreur "(.*)"/, (errorMessage: string) => {
  ...
  test('Le format de réponse est extensible pour futures permissions', ({ given, when, then, and }) => {
    givenAuthenticatedUser(given);
