...
describe('DigestionJobConsumer Integration Tests (Task 5.6)', () => {
  ...
  beforeEach(async () => {
    ...
    const mockOpenAIService = {
      digestContent: jest.fn().mockResolvedValue({
        summary: 'This is a test summary generated by GPT for the content provided.',
        ideas: [
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: {
            processContent: jest.fn().mockImplementation(async (content, contentType) => {
        ...
        },
        {
          ...
          useValue: {
            createWithIdeas: jest.fn().mockImplementation(
              async (captureId, userId, summary, ideas, processingTimeMs, confidenceScore) => {
    ...
    contentExtractor = module.get<ContentExtractorService>(ContentExtractorService);
  ...
  describe('Full Digestion Flow (Task 5.6)', () => {
    it('should complete full digestion flow from job to Thought creation', async () => {
      ...
      // 1. Content extracted
      expect(contentExtractor.extractContent).toHaveBeenCalledWith('capture-123');
      ...
      // 5. Events published
      const startedEvent = publishedEvents.find(e => e.event === 'digestion.job.started');
      ...
      const completedEvent = publishedEvents.find(e => e.event === 'digestion.completed');
    ...
    it('should handle audio capture digestion flow', async () => {
      ...
      // Assert
      expect(contentExtractor.extractContent).toHaveBeenCalledWith('audio-capture-456');
      expect(contentChunker.processContent).toHaveBeenCalledWith(
    ...
    it('should propagate errors through the flow', async () => {
      ...
      // Act & Assert
      await expect(consumer.handleDigestionJob(job)).rejects.toThrow('Content extraction failed');
    ...
    it('should record processing time in Thought entity', async () => {
      ...
      const createCall = (thoughtRepository.createWithIdeas as jest.Mock).mock.calls[0];
  ...
  describe('Error Handling Integration (Task 5.6)', () => {
    it('should update capture status to digestion_failed after max retries', async () => {
      ...
      // Act & Assert
      await expect(consumer.handleDigestionJob(job)).rejects.toThrow('GPT API timeout');
      ...
      const failedEvent = publishedEvents.find(e => e.event === 'digestion.job.failed');
    ...
    it('should not update to failed status before max retries', async () => {
      ...
      // Act & Assert
      await expect(consumer.handleDigestionJob(job)).rejects.toThrow('Temporary error');

      // Should NOT update to digestion_failed yet
      const failedStatusCalls = (captureRepository.updateStatus as jest.Mock).mock.calls.filter(
        call => call[1] === 'digestion_failed'
      );
